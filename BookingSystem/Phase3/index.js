...
const resourceAction = String(action).trim();
  ...
  const description = String(resourceDescription).trim(); // KORJAUS 5: Älä tyhjennä kuvausta
...
return res.json({ 
    ok: true, 
    echo: { ...req.body, resourceDescription: description } 
  });
}); // <--- TÄMÄ SULKU PUUTTUI! Se sulkee app.post-funktion.

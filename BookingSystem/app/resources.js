...
const resourcePriceUnitInputs = document.getElementsByName("resourcePriceUnit");
...
function attachResourceNameValidation(input) {
  const update = () => {
    // Päivitä vain inputin visuaalinen tila
    const valid = isResourceNameValid(input.value);
    setInputVisualState(input, valid ? "valid" : (input.value ? "invalid" : "neutral"));

    // Kutsu master-funktiota, joka tarkistaa kaikki kentät
  ...
  // Alusta tila sivun latauksessa
  update();
}


//master validointi funktio
function updateCreateButtonState() {
  // 1. Name valid
  const nameValid = isResourceNameValid(resourceNameInput.value);

  // 2. Description valid (10-50 merkkiä)
  const descValue = resourceDescriptionInput.value.trim();
  const descValid = descValue.length >= 10 && descValue.length <= 50;

  // 3. Price valid (numero >= 0)
  const priceValue = parseFloat(resourcePriceInput.value);
  const priceValid = !isNaN(priceValue) && priceValue >= 0;

  // 4. Price unit selected
  const priceUnitValid = Array.from(resourcePriceUnitInputs).some(r => r.checked);

  // 5. Kaikki valid
  const allValid = nameValid && descValid && priceValid && priceUnitValid;

  // Päivitä Create-painike
  setButtonEnabled(createButton, allValid);

  // Päivitä kenttien värit
  setInputVisualState(resourceNameInput, nameValid ? "valid" : (resourceNameInput.value ? "invalid" : "neutral"));
  setInputVisualState(resourceDescriptionInput, descValid ? "valid" : (descValue ? "invalid" : "neutral"));
}
...
// Liitä master-validointi myös muihin kenttiin
resourceDescriptionInput.addEventListener("input", updateCreateButtonState);
resourcePriceInput.addEventListener("input", updateCreateButtonState);
resourceAvailableInput.addEventListener("change", updateCreateButtonState);
resourcePriceUnitInputs.forEach(r => r.addEventListener("change", updateCreateButtonState));
